...
.ai/bugs/bugindex.json               # bug-index-manifest-v2
...
- `.ai/bugs/bugindex.json` is the machine-readable manifest and contains the same lightweight lookup fields plus `shard`.
...
Create the first manifest and shard only when the project has its first reproducible bug. When migrating an existing flat record, preserve IDs, create one shard per record, reduce `.ai/bugs/bugindex.json` to the manifest, and update `.ai/bugs/BUG_INDEX.md` in the same change.

## Before Debugging

1. Search `.ai/bugs/bugindex.json` by symptom, error text, feature, file, symbol, and related bug ID.
...
Record the confirmed cause, exact fix, changed symbols, and actual verification in the shard. Update the summary fields in `.ai/bugs/bugindex.json` and `.ai/bugs/BUG_INDEX.md`. Set `VERIFIED` only after the defined checks pass.
...
- Update the affected shard, `.ai/bugs/bugindex.json`, and `.ai/bugs/BUG_INDEX.md` whenever a bug changes state.
